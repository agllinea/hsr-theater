...
const CROSSFADE_INTERSECTION = 8;
    ...
    const setupAudioEvents = (audio: HTMLAudioElement) => {
        ...
        audio.ontimeupdate = () => {
            ...
            if (crossfadeActiveRef.current && nextAudioRef.current) {
                const fadeProgress = Math.max(0, Math.min(1, 1 - remaining / CROSSFADE_INTERSECTION));
        ...
        const tick = () => {
            ...
            if (duration && !isNaN(duration) && duration > 0) {
                ...
                if (progressBarRef.current) {
                    const scale = playCountRef.current % 2 === 1 ? 1 - pct / 100 : pct / 100;
