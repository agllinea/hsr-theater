...
import "./CrystalEntry.css";

const BTN_W = 224;
const BTN_H = 62;

const BASE_COLOR: [number, number, number] = [189, 122, 255];
...
interface CrystalEntryProps {
    onClick: () => void;
}

export function CrystalEntry({ onClick }: CrystalEntryProps) {
    const svgPoints = useRef(makeCrystal());

    const [r, g, b] = BASE_COLOR;
    const o = OPACITY_BOOST;
    ...
    return (
        <div className="crystal-entry" onClick={onClick}>
            <motion.div
                className="crystal-entry__crystal"
                animate={{
                    scale: [1, 1.045, 1],
                    filter: [
            ...
            >
                <svg viewBox={`0 0 ${BTN_W} ${BTN_H}`} className="crystal-entry__svg">
                    <defs>
                    ...
                    <polygon points={svgPoints.current} fill="url(#lg-crystal-body)" />
                    <polygon points={svgPoints.current} fill="url(#lg-crystal-sheen)" />
                    <polygon points={svgPoints.current} fill="none" stroke={colL(0.55)} strokeWidth={1} />
                    <polygon points={svgPoints.current} fill="none" stroke={colL(0.88)} strokeWidth={0.5} />
                </svg>
            </motion.div>
            <span className="crystal-entry__label">See You Tomorrow</span>
        </div>
    );
}
