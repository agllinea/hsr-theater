...
export const Legends = () => {
    ...
    const handleLegendClick = (code: string | null) => {
        if (code === null) {
            setOpenLegend(false);
        } else {
    ...
    return (
        <div className="legend-container">
            <div className="legend-lables">
                {legends.map((legend, index) => (
                    <div
                        ...
                        onMouseEnter={() => handleLegendClick(legend.code)}
            ...
            <motion.div
                ...
                animate={{ height: openLegend ? "auto" : "0", opacity: 1 }}
                transition={{ duration: 0.3, delay: 0.2 }}
            >
                <span
                    className="legend-detail"
                    dangerouslySetInnerHTML={{
                        __html: (currentLegend && legends.find((l) => l.code === currentLegend)?.description) ?? "",
                    }}
                ></span>
            </motion.div>
            {/* <AnimatePresence>
                {currentLegend && (
                    <motion.div
                        className="legend-detail-container"
                        initial={{ height: 0, opacity: 0 }}
                    ...
                    >
                        <div
                            ...
                            dangerouslySetInnerHTML={{
                                __html:
                                    (currentLegend && legends.find((l) => l.code === currentLegend)?.description) ?? "",
                            }}
                        ></div>
                    </motion.div>
                )}
