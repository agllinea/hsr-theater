...
import HandWriting from "../animated_components/hand_writing";
...
export default function Main() {
    ...
    const subpages = [
        <>
            <div className="subpage  main-container flex-center">
                <span className="t0">
                    <HandWriting
                        paths={[
                            {
...
const containerRef = useRef(null);
  ...
  const y = useMotionValue(0); // current scroll position
  ...
  useEffect(() => {
    ...
    const handleWheel = (e) => {
      e.preventDefault();
      y.set(y.get() + e.deltaY); // add scroll delta to y motion value
      // clamp
      if (y.get() < 0) y.set(0);
      if (y.get() > (subpages.length - 1) * pageHeight) y.set((subpages.length - 1) * pageHeight);
    };
    ...
    const handleTouchMove = (e) => {
      const delta = startTouchY.current - e.touches[0].clientY;
      y.set(y.get() + delta);
      startTouchY.current = e.touches[0].clientY;
      // clamp
      if (y.get() < 0) y.set(0);
  ...
  }, [y, subpages.length, pageHeight]);

  // --- Calculate opacity of each page based on scroll ---
  const getOpacity = (index) => {
    const start = index * pageHeight;
    const end = (index + 1) * pageHeight;
    const value = y.get();

    if (value <= start) return 1;
    if (value >= end) return 0;
    return 1 - (value - start) / pageHeight; // fade out as we scroll to next page
  };

  // --- Calculate translateY for parallax / next page effect ---
  const getTranslate = (index) => {
    const value = y.get();
    return index * pageHeight - value;
  };
      ...
      {subpages.map((content, i) => (
        <motion.div
          ...
          style={{
            ...
            alignItems: "center",
            fontSize: "4rem",
            background: ["#ffadad", "#ffd6a5", "#fdffb6", "#caffbf"][i % 4],
            y: getTranslate(i),
